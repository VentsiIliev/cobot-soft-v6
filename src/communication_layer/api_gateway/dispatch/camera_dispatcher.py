...
class CameraDispatch(IDispatcher):
    ...
    def dispatch(self, parts: list, request: str, data: dict = None) -> dict:
        ...
        elif request in [camera_endpoints.CAMERA_ACTION_GET_WORK_AREA_POINTS]:
            return self.handle_get_work_area_points(data)
    ...
    def handle_get_work_area_points(self, data):
        """
        Handle getting work area points.
        
        Args:
            data: Request data containing area_type
            
        Returns:
            dict: Response with work area points data or error
        """
        print(f"CameraHandler: Handling get work area points: {data}")
        ...
        try:
            ...
            if isinstance(data, dict):
                ...
                # If data is a string, assume it's the area_type directly
                area_type = data
            else:
                area_type = None
            ...
            if not area_type:
                return Response(
                    Constants.RESPONSE_STATUS_ERROR, 
                    message="area_type parameter is required"
                ).to_dict()
            
            # Pass the data as expected by the controller
            request_data = {'area_type': area_type} if isinstance(data, str) else data
            points = self.cameraSystemController.getWorkAreaPoints(request_data)
            
            return Response(
                Constants.RESPONSE_STATUS_SUCCESS, 
                message=f"Work area points retrieved successfully for {area_type} area",
                data=points
            ).to_dict()
            ...
            import traceback
            traceback.print_exc()
            print(f"CameraHandler: Error getting work area points: {e}")
            return Response(
                Constants.RESPONSE_STATUS_ERROR, 
                message=f"Error getting work area points: {e}"
            ).to_dict()
