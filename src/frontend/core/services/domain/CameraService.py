...
class CameraService:
    ...
    def get_work_area_points(self, area_type: str) -> ServiceResult:
        """
        Get work area calibration points for the specified area type.
        
        Args:
            area_type: The area type ('pickup', 'spray', or 'work')
        
        Returns:
            ServiceResult with loaded work area points or error
        """
        try:
            if not area_type or area_type not in ['pickup', 'spray', 'work']:
                return ServiceResult.error_result("Invalid area type. Must be 'pickup', 'spray', or 'work'")
            
            self.logger.info(f"Loading work area points for area type: {area_type}")
            
            points = self.controller.handleGetWorkAreaPoints(area_type)
            
            if points is None:
                return ServiceResult.success_result(
                    f"No saved points found for {area_type} area",
                    data=None
                )
            
            return ServiceResult.success_result(
                f"Work area points loaded successfully for {area_type} area ({len(points)} points)",
                data=points
            )
                
        except Exception as e:
            error_msg = f"Failed to get work area points for {area_type}: {str(e)}"
            self.logger.error(error_msg, exc_info=True)
            return ServiceResult.error_result(error_msg)
