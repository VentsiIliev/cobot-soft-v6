...
# Module-level configuration - will be set by configure_workpiece_fields()
_workpiece_field_config: Optional['WorkpieceFieldConfig'] = None
_workpiece_factory: Optional[callable] = None


def configure_workpiece_fields(config: Optional['WorkpieceFieldConfig'], factory: Optional[callable] = None):
    """
    Configure workpiece form fields from a WorkpieceFieldConfig object.
    Must be called before any CreateWorkpieceForm is created.

    Args:
        config: WorkpieceFieldConfig instance with field definitions (can be None for minimal config)
        factory: Optional factory function to create workpiece objects from form data
    """
    global _workpiece_field_config, _workpiece_factory
    _workpiece_field_config = config
    _workpiece_factory = factory

    if config:
        print(f"[CreateWorkpieceForm] Configured with {len(config.fields)} fields")
    else:
        print("[CreateWorkpieceForm] Configured with minimal config (factory only)")


def get_workpiece_field_config() -> Optional['WorkpieceFieldConfig']:
    """Get the current workpiece field configuration (may be None)"""
    return _workpiece_field_config


def get_workpiece_factory() -> Optional[callable]:
    """Get the workpiece factory if configured"""
    return _workpiece_factory
