...
class RobotService(IRobotService):
    ...
    def robot_config(self):
        return self.settings_service.get_robot_config()
    ...
    def start_jog(self, axis:RobotAxis, direction:Direction, step):
        # if axis is not Axis instance, raise error
        if not isinstance(axis, RobotAxis):
            raise ValueError(f"Invalid axis: {axis}. Must be an instance of RobotAxis enum.")


        if not isinstance(direction, Direction):
            raise ValueError(f"Invalid direction: {direction}. Must be an instance of Direction enum.")
