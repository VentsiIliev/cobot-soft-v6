...
from applications.glue_dispensing_application.handlers import spraying_handler
...
class GlueSprayingApplication(BaseRobotApplication, RobotApplicationInterface):
    ...
    def __init__(self,
        ...
        self.pick_and_place_operation = PickAndPlaceOperation()
        self.pick_and_place_operation.set_state_publisher(OperationStatePublisher(self.broker))
        ...
        self.initialize_glue_data_fetcher()
    ...
    def start_nesting(self, debug=True)-> OperationResult:
        # Switch to pick-and-place operation
        self.current_operation = self.pick_and_place_operation

        # Start the operation and get the actual result
        result = self.pick_and_place_operation.start(self, self.get_workpieces())

        # Return the actual result (success/stopped/error)
        return result

    def start_spraying(self,workpieces, debug=True)-> OperationResult:
        # Switch to glue dispensing operation
        self.current_operation = self.glue_dispensing_operation
    ...
    def move_to_spray_capture_position(self):
        z_offset = self.settingsManager.get_camera_settings().get_capture_pos_offset()
        print(f"move_to_spray_capture_position -> z_offset: {z_offset}")
